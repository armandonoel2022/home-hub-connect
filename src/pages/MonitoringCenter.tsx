...
import {
  ArrowLeft, Monitor, Clock, CalendarIcon, UserCheck, AlertTriangle,
...
interface MonitoringReport {
  ...
  hours?: number;
  description: string;
  coveringFor?: string; // userId being covered
  createdBy: string;
  createdAt: string;
  status: "Pendiente RRHH" | "Asignada" | "Procesada";
  assignedTo?: string; // RRHH staff
}
...
const reportConfig: { key: ReportType; label: string; icon: any; color: string; description: string }[] = [
  ...
  { key: "ausencia", label: "Ausencia", icon: UserX, color: "hsl(0 60% 50%)", description: "Reportar ausencia de un operador" },
  { key: "cobertura", label: "Cobertura de Turno", icon: Repeat, color: "hsl(160 60% 40%)", description: "Reportar cobertura de turno de otro operador" },
];

const MonitoringCenter = () => {
  const navigate = useNavigate();
  const { user, allUsers, activeUsers } = useAuth();
  ...
  const monitoringOperators = activeUsers.filter(
    (u) => u.department === "Tecnología y Monitoreo" && u.position?.toLowerCase().includes("operador")
  );

  // Group by team
  const teams = monitoringOperators.reduce<Record<string, typeof monitoringOperators>>((acc, op) => {
    const team = op.team || "Sin Equipo";
    if (!acc[team]) acc[team] = [];
    acc[team].push(op);
    return acc;
  }, {});

  // RRHH staff for assignment
  const rrhhStaff = activeUsers.filter((u) => u.department === "Recursos Humanos");

  const handleSubmitReport = () => {
    if (!selectedOperator || !reportDate || !activeReport) return;
    const operator = allUsers.find((u) => u.id === selectedOperator);
    if (!operator) return;

    const coveringForUser = coveringForId ? allUsers.find((u) => u.id === coveringForId) : null;

    const newReport: MonitoringReport = {
      id: `MON-${String(Date.now()).slice(-6)}`,
      type: activeReport,
      operatorId: selectedOperator,
      operatorName: operator.fullName,
      date: format(reportDate, "yyyy-MM-dd"),
      hours: reportHours ? parseFloat(reportHours) : undefined,
      description: reportDescription || getDefaultDescription(activeReport, operator.fullName, coveringForUser?.fullName),
    ...
    let notifMessage = `${operator.fullName} — ${typeLabel} el ${format(reportDate, "dd/MM/yyyy")}`;
    if (newReport.hours) notifMessage += ` (${newReport.hours}h)`;
    if (coveringForUser) notifMessage += `. Cubriendo a: ${coveringForUser.fullName}`;

    // Notify RRHH (Dilia - USR-006)
    addNotification({
      type: "info",
      title: `Reporte de Monitoreo: ${typeLabel}`,
    ...
    // For overtime, holidays, worked days off — also alert with overlay-triggering notification
    if (["horas-extras", "dia-feriado", "dia-libre-trabajado"].includes(activeReport)) {
      addNotification({
        type: "info",
        title: `⚠️ REPORTE MONITOREO: ${typeLabel.toUpperCase()}`,
        message: notifMessage,
        relatedId: newReport.id,
        forUserId: "USR-006",
        actionUrl: "/monitoreo",
  ...
  const getDefaultDescription = (type: ReportType, name: string, coveringName?: string) => {
    switch (type) {
      ...
      case "cobertura": return `${name} cubrió el turno de ${coveringName || "otro operador"}`;
    }
  };
  ...
  const handleAssignToRRHH = (reportId: string, staffId: string) => {
    ...
    if (report && staff) {
      addNotification({
        type: "info",
        title: "Tarea Asignada — Monitoreo",
        message: `${report.operatorName}: ${reportConfig.find((c) => c.key === report.type)?.label} — Asignada a ${staff.fullName}`,
        relatedId: reportId,
        forUserId: staffId,
        actionUrl: "/monitoreo",
      });
  ...
  const isTechMonitoreo = user?.department === "Tecnología y Monitoreo" || user?.isAdmin;
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Gestión de operadores, horarios y reportes
              </p>
            </div>
          </div>
          ...
          {!activeSection && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              >
                ...
                <span className="font-heading font-bold text-card-foreground">Operadores</span>
                <span className="text-xs text-muted-foreground text-center">
                  Ver operadores, equipos y horarios
                </span>
                <span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-semibold">
                  {monitoringOperators.length} operadores
                </span>
              </button>
              {isTechMonitoreo && (
                <button
              ...
              {Object.entries(teams).map(([teamName, members]) => (
                <div key={teamName} className="bg-card rounded-xl border border-border overflow-hidden">
                  <div className="px-5 py-3 bg-muted flex items-center gap-2">
                    ...
                    <h3 className="font-heading font-bold text-card-foreground text-sm">{teamName}</h3>
                    <span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary ml-auto">
                      {members.length} operadores
                    </span>
                  </div>
                    ...
                    {members.map((op) => (
                      <div key={op.id} className="px-5 py-3 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-card-foreground">{op.fullName}</p>
                          <p className="text-xs text-muted-foreground">{op.position}</p>
                        </div>
                        {op.shift && (
                          <div className="flex items-center gap-1.5 text-xs">
                            {op.shift.toLowerCase().includes("noche") ? (
                              <Moon className="h-3.5 w-3.5 text-indigo-400" />
                            ) : op.shift.toLowerCase().includes("tarde") ? (
            ...
            <div className="max-w-lg mx-auto bg-card rounded-xl border border-border p-6 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={selectedOperator} onValueChange={setSelectedOperator}>
                  ...
                  <SelectContent>
                    {monitoringOperators.map((op) => (
                      <SelectItem key={op.id} value={op.id}>
                        {op.fullName} — {op.team || "Sin equipo"} ({op.shift || "Sin turno"})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {activeReport === "cobertura" && (
                <div className="space-y-1.5">
                  ...
                  <Select value={coveringForId} onValueChange={setCoveringForId}>
                    ...
                    <SelectContent>
                      {monitoringOperators.filter((op) => op.id !== selectedOperator).map((op) => (
                        <SelectItem key={op.id} value={op.id}>
                          {op.fullName} — {op.team || "Sin equipo"}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              <div className="flex gap-3 pt-2">
                <Button variant="outline" className="flex-1" onClick={() => setActiveReport(null)}>
                  Cancelar
                </Button>
                <Button
                  className="flex-1"
                  disabled={!selectedOperator || !reportDate}
                ...
                reports.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-card-foreground">{r.operatorName}</p>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        <p className="font-semibold text-card-foreground">{showDetail.operatorName}</p>
                      </div>
                      <div>
                      ...
                      {showDetail.coveringFor && (
                        <div>
                          ...
                          <p className="font-semibold text-card-foreground">
                            {allUsers.find((u) => u.id === showDetail.coveringFor)?.fullName || showDetail.coveringFor}
                          </p>
                        </div>
                      )}
                      <div>
